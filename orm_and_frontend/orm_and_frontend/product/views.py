...
#     for data in cart:
#         order= Order_Table.objects.create(order_id=oid,quantity=data.quantity,pid=data.pid,uid=data.uid)
#         order.save()
...
def make_payment(request):
    ...
    client = razorpay.Client(auth=("rzp_test_jyUYCzPqd2Ej6a", "DtYIx4bn9Qm59NNzfYwheYWu"))

    data = { "amount": total_price*100, "currency": "INR", "receipt": "order_rcptid_11" }
    payment = client.order.create(data=data)
    print(payment)
    return HttpResponse(request,'product/pay.html',context=data)